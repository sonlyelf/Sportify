...
public class TradeDaoImpl implements TradeDao {
	...
	public Trade findById(Integer id) {
		...
		String sql = "SELECT id, user_id, course_id, createDate, course_price, payment_status, order_status "
					+ "FROM trade "
					+ "WHERE id=?";
	...
	public int createTrade(Trade trade) {
		...
		String sql = "INSERT INTO trade (user_id, course_id, createDate, course_price, payment_status, order_status ) "
					+ "VALUES (?, ?, ?, ?, ?, ?);";
		
		return jdbcTemplate.update(sql, 
				trade.getUserId(), trade.getCourseId(), trade.getCreateDate(), trade.getCoursePrice(), 
	...
	public int updateTrade(Trade trade) {
		...
		String sql = "UPDATE trade "
					+ "SET user_id=?, course_id=?, createDate=?, course_price=?, payment_status=?, order_status=? "
					+ "WHERE id=?";
		
		return jdbcTemplate.update(sql, trade.getUserId(), trade.getCourseId(), trade.getCreateDate(),
				trade.getCoursePrice(), trade.getPaymentStatus(), trade.getOrderStatus(), trade.getId());
	}
	 ...
	 * 
	 * @Override public Trade mapRow(ResultSet rs, int rowNum) throws SQLException {
	 * Trade trade = new Trade(); trade.setId(rs.getInt("id"));
	 ...
	 * trade.setOrderStatus(rs.getString("order_status")); return trade; }
	 * }
	 */
}
